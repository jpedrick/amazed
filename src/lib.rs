...
mod tests {
    ...
    fn test_insert() {
        ...
        let s = graph::GraphNodes::<usize, 6>::from([
            (
                1,
                (&[Some(2), Some(3), Some(4), Some(5), None, None]).into(),
            ),
    ...
    fn test_add_edge() {
        ...
        assert_eq!(
            g.insert_edge(5, 2).err(),
            Some(crate::graph::InsertEdgeError::AppendEdgeError(
                crate::graph::AddEdgeError::Exists
            ))
        );
    ...
    fn test_overflow_node() {
        ...
        assert_eq!(
            g.insert_edge(1, 8).err(),
            Some(crate::graph::InsertEdgeError::AppendEdgeError(
                crate::graph::AddEdgeError::Overflow
            ))
        );
        assert_eq!(
            g.insert_edge(9, 1).err(),
            Some(crate::graph::InsertEdgeError::FromMissing)
        );
    }
