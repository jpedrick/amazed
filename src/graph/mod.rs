...
impl<T: KeyTraits, const MAX_NEIGHBORS: usize> Neighbors<T, MAX_NEIGHBORS> {
    pub fn append(&mut self, node_id: T) -> Result<&mut Self, AddEdgeError> {
        if self.1 < MAX_NEIGHBORS {
            match self
                .0
                .binary_search_by(|x| option_less_than(x, &Some(node_id)))
            {
...
impl<T: KeyTraits, const N: usize, const MAX_NEIGHBORS: usize> From<&[T; N]>
    for Neighbors<T, MAX_NEIGHBORS>
{
...
impl<T: KeyTraits, const MAX_NEIGHBORS: usize> From<&[Option<T>; MAX_NEIGHBORS]>
    for Neighbors<T, MAX_NEIGHBORS>
{
    fn from(neighbors: &[Option<T>; MAX_NEIGHBORS]) -> Neighbors<T, MAX_NEIGHBORS> {
        let first_null = neighbors
            .iter()
